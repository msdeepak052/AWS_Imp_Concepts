...
> Goal: make the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's central idea — the **cache key** decides "sameness," the **Origin Request Policy** decides what the origin *sees*, and these are two independent, decoupled controls — something you can actually **watch happen**, not just read about. Entirely via the **AWS Console**, no CLI.

> 🧠 **Cost note, corrected after live testing:** the CloudFront console shows an "Upgrade to Business" banner next to custom cache policies (visible on the Create/Edit behavior page). That banner turned out to be an **upsell nudge, not an actual paywall** — creating and using a custom cache policy (Sections 5-6 below) worked fine on a plain **Pay-as-you-go** distribution, no Business plan ($200/month) required. If your account ever does hit a genuine hard block creating one, that's the point to check your plan — but don't pre-emptively upgrade for this lab; it isn't necessary.

---

## 1. Why this demo needs a second origin

The S3 origin from the [CloudFront Hands-On Lab 1 (S3 static site + CDN)](02-CloudFront-HandsOn-Lab1.md) note always returns the exact same bytes for `index.html`, no matter what query string or header you send — so it can't visually prove anything about cache keys. A request for `/index.html?id=1` and `/index.html?id=2` would *look* identical either way, bug or no bug.
...
> 🧠 **Mental model:** think of `postman-echo.com/get` as a mirror. If CloudFront serves you a cached response, the mirror shows you what an *earlier* request looked like — not necessarily your own. That mismatch **is** the cache-key bug from the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note, Section 1, made visible.

This demo is the mechanical proof behind the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's Section 2 **two-leg model** (Viewer → Edge is the Cache Key check; Edge → Origin, on a Miss only, is the Origin Request Policy) and Section 3's Netflix walkthrough — same two controls, just isolated here with a tool that lets you see both legs directly instead of inferring them from video quality/language.

---

## 2. Prerequisite

The CloudFront distribution + S3 bucket from the [CloudFront Hands-On Lab 1 (S3 static site + CDN)](02-CloudFront-HandsOn-Lab1.md) note, still deployed. If you already cleaned that up, just recreate a minimal distribution (any origin works as the *default* behavior — this demo only touches a new, separate cache behavior, so what the default origin is doesn't matter here).
...
7. **Function associations — optional**: leave all four rows (Viewer request, Viewer response, Origin request, Origin response) at **No association** — this is CloudFront Functions/Lambda@Edge territory (the [CloudFront Function Associations](11-CloudFront-Function-Associations.md) note), not needed here.
8. **Create behavior**. Wait for the distribution to show **Deployed** again.

> 🧠 You'll notice a **"Custom cache policies — Upgrade to Business"** banner sitting right above the Cache policy dropdown on this same page. It's the same upsell banner the cost note at the top of this lab refers to — it's an AWS marketing nudge, not an actual restriction; Section 5 creates a genuinely custom policy on a Pay-as-you-go distribution without issue.
...
The second request asked for `id=2` from `bob`, but CloudFront treated it as **the same cache entry** as the first request (cache key = path only, `/get`, ignoring the query string and header entirely) — Leg 1 (the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's Section 2) returned a Hit. Because Leg 1 was a Hit, **Leg 2 never ran** — `AllViewerExceptHostHeader` never got the chance to forward `id=2`/`bob` anywhere, since there was no miss to trigger it. This is the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's Section 1's warning, no longer theoretical.

---

<img width="2557" height="628" alt="image" src="https://github.com/user-attachments/assets/eb424e3a-903a-452a-a848-3ac85ee13c7c" />
...
<img width="2544" height="366" alt="image" src="https://github.com/user-attachments/assets/89e7cb40-6a0a-42eb-a037-ea3065ce0fe0" />
...
<img width="2556" height="1440" alt="image" src="https://github.com/user-attachments/assets/0d115793-deb6-40c6-b88d-02cf090951e8" />
...
Every fix that made the response **more correct** also made the cache **less shared** — 1 slot became 3 slots for the same 3 requests. In a real system with thousands of distinct `id` values and header values, this is exactly the hit-ratio cost the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's Section 2 describes: each additional cache-key dimension is a deliberate trade of hit ratio for correctness, not a free upgrade.

---

## 8. Managed vs. custom policy, in this demo

- **`Managed-CachingOptimized`** and **`Managed-AllViewerExceptHostHeader`** (Section 3) are **AWS managed policies** — ready-made for the common cases, no authoring needed (the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's Section 6).
...
4. Leave the rest of the distribution/S3 bucket in place if you're continuing to other hands-on notes in this folder — only tear those down per the [CloudFront Hands-On Lab 1 (S3 static site + CDN)](02-CloudFront-HandsOn-Lab1.md) note's own cleanup section when you're done with the whole series.
