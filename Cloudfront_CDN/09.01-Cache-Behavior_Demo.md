...
1. **CloudFront console** → **Policies** → **Cache policies** → **Create cache policy**.
2. **Name**: `Demo-CacheKey-Custom`.
3. **TTL settings**: leave defaults.
4. **Cache key settings**:
   - **Query strings**: **Include specified query strings** → add `id`.
...
1. **Policies** → **Cache policies** → select `Demo-CacheKey-Custom` → **Edit**.
2. **Headers**: switch to **Include specified headers** → add `X-Demo-User`.
...
3. **Policies** → **Cache policies** → select `Demo-CacheKey-Custom` → **Delete** (only possible once no behavior references it).
