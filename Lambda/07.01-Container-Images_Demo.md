...
docker tag lambda-container-demo:latest \
  <your-account-id>.dkr.ecr.ap-south-1.amazonaws.com/lambda-container-demo:latest

docker push <your-account-id>.dkr.ecr.ap-south-1.amazonaws.com/lambda-container-demo:latest
```
...
5. **Architecture**: leave at **x86_64** (must match what your local Docker build produced — see Section 9's troubleshooting if this mismatches).
6. Leave the rest at their defaults (the same auto-created basic execution role every other function in this folder has used).
