...
> ⚠️ **Why `AllViewerExceptHostHeader` and not plain `AllViewer`:** the plain `Managed-AllViewer` policy forwards *every* viewer header to the origin — including `Host`. Since viewers hit this distribution at `<your-id>.cloudfront.net`, that's the `Host` value CloudFront would forward to `postman-echo.com` — a third-party origin that doesn't recognize your CloudFront domain and will reject or mishandle the connection, which CloudFront reports back as a `502 Bad Gateway`. This is a genuine, common real-world CloudFront gotcha whenever your origin isn't your own infrastructure (an S3 bucket or your own ALB, which don't care about Host) — AWS ships `AllViewerExceptHostHeader` specifically to solve it: identical forwarding behavior, minus `Host`, so CloudFront sets it correctly to the origin's own domain instead.
