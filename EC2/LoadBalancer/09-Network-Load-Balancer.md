...
> Goal: understand **NLB** — the Layer 4 sibling of `demo-alb` — and recognize the specific scenarios (static IPs, source IP preservation, extreme scale/latency, non-HTTP protocols) where the exam expects NLB instead of ALB. Concepts-only; Note 10 builds `demo-nlb` for real.

---

## 1. What is a Network Load Balancer?

A **Network Load Balancer (NLB)** operates at **Layer 4** of the OSI model — it load-balances **TCP, UDP, and TLS** connections. Unlike `demo-alb`, it does **not** parse HTTP requests, look at paths, headers, or hostnames, and has no concept of listener rules with conditions like `path-pattern` or `host-header`. It simply forwards **connections** (or, for UDP, packet flows) to a healthy target based on a flow hash — it doesn't understand or care what's inside them.

> 🧠 **Mental model:** if `demo-alb` is a smart receptionist reading the contents of every visitor's request before deciding where to send them, an NLB is a **switchboard operator** — it just connects call A to line B as fast and as reliably as physically possible, without listening in on the conversation.

---

## 2. Why use NLB instead of ALB?

`demo-alb` is the right default for HTTP/HTTPS web applications. NLB earns its place when one of these specific needs shows up:
...
## 4. Diagram: `demo-nlb` — TCP passthrough to a demo instance

```mermaid
flowchart TD
    CLIENT(("TCP client")) -->|"TCP:5000"| NLB["demo-nlb<br/>internet-facing<br/>listener TCP:5000"]

    subgraph VPC["Your VPC"]
        NLB --> TG["demo-nlb-tg<br/>target group, TCP:5000"]
        TG --> I1["demo-tcp-1<br/>private subnet 1<br/>listening on 5000"]
    end
```

Note 10 builds this exact pair: `demo-nlb` fronting a target group `demo-nlb-tg` that forwards to a single demo instance `demo-tcp-1`.

---

## 5. Listener protocols in more detail

An NLB listener (like the `TCP:5000` one Note 10 builds for `demo-nlb`) can be one of:

| Listener protocol | What it does |
|---|---|
| **TCP** | Pure Layer 4 passthrough — no inspection, no termination. What `demo-nlb` uses. |
...
`demo-nlb`'s listener stays plain **TCP** since `demo-tcp-1` is a raw, unencrypted demo service — no certificate involved.
...
Health checks work similarly to ALB target groups conceptually (healthy/unhealthy thresholds, interval, protocol/port) but the check itself can be plain **TCP** (just "can I open a connection on this port?") or **HTTP/HTTPS** if the target exposes a real health endpoint — `demo-nlb-tg` in Note 10 uses a simple TCP check since `demo-tcp-1` has no HTTP endpoint to probe.
...
🎯 **Exam tip:** cross-zone load balancing is **always on and free** for ALB, but **off by default and billed for inter-AZ data transfer** if you turn it on for NLB — a favorite trick in cost-optimization scenario questions.
...
- Next: Note 10 builds `demo-nlb` + `demo-nlb-tg` + `demo-tcp-1` end-to-end in the console, including attaching an Elastic IP per AZ.
