...
6. **Cache key and origin requests** (the page defaults to the recommended cache-policy/origin-request-policy mode already):
   ...
   - **Origin request policy — optional**: defaults to unselected — choose `Managed-AllViewer` (forwards **all** viewer headers, cookies, and query strings to the origin, regardless of what's in the cache key).
