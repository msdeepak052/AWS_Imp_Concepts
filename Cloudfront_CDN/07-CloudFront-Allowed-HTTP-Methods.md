...
> Goal: understand the **Allowed HTTP Methods** cache-behavior setting — which verbs CloudFront will even forward to the origin at all — and why this setting works hand-in-hand with the [CloudFront Origin Access](06-CloudFront-Origin-Access.md) note's OAC to let CloudFront proxy uploads, not just reads.
...
> 🧠 **Mental model:** "Allowed HTTP Methods" answers *which verbs get through CloudFront at all*; caching behavior (the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note) separately answers *which of those get served from a cached copy* — and the answer to the second question is always "only the read-only ones," regardless of the first setting.
...
| Distribution serving both static assets **and** an API (multiple cache behaviors, the [Default Cache Behavior Options](04-Default-Cache-Behavior-Options.md) note) | GET/HEAD-only on the static-asset cache behavior; full method set on the `/api/*` cache behavior |
...
- **Allowed HTTP Methods** is a hard gate at the CloudFront layer — a disallowed method never reaches the origin, regardless of what the origin or OAC (the [CloudFront Origin Access](06-CloudFront-Origin-Access.md) note) would otherwise permit.
- Even with all methods allowed, **only GET/HEAD responses are ever cached** — everything else is always forwarded live.
- A distribution serving both static content and a dynamic API typically needs **separate cache behaviors** (the [Default Cache Behavior Options](04-Default-Cache-Behavior-Options.md) note) with different method sets for each path pattern.
- Next: the [Restrict Viewer Access](08-Default-Cache-Behavior-Restrict-Viewer-Access.md) note, controlling *who* (not just *what verbs*) can reach the content at all.
