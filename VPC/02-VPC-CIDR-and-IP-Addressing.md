...
Full binary math and all the worked subnetting examples live in **Note 03** — this note only needs the "bigger block = smaller prefix number" intuition.
...
- You **cannot** create a VPC smaller than `/28` — you need at least a few usable addresses per subnet, and AWS itself reserves 5 IPs per subnet (Note 03 §2), so anything smaller is nearly useless.
- Our worked example VPC, `myapp-vpc`, uses `10.0.0.0/16` — the largest, most flexible choice, giving room for many subnets later (Note 03 §4 shows exactly how much is left over).
...
- Instances in the VPC still get **outbound internet access** via a public IP or Elastic IP mapped on top (NAT, Note 09) — the private CIDR itself never needs to be "public."
...
- More on secondary CIDRs with worked examples in **Note 03 §8**.
...
Suppose `myapp-vpc` uses `10.0.0.0/16`, and later your company builds a second VPC `myapp-vpc-b` also using `10.0.0.0/16`, then tries to **peer** them (Note 11).
...
The same logic applies to **Site-to-Site VPN** (Note 15): if your on-premises network is `10.0.0.0/16` too, your on-prem servers and your VPC subnets have colliding addresses, and routing simply cannot tell them apart.
...
Each subnet is carved out of the VPC's `/16` block, leaving plenty of address space unused for future tiers (e.g. a database subnet, Note 07). The exact arithmetic behind this layout is worked out in full in Note 03.
...
- Our example: `myapp-vpc` = `10.0.0.0/16`, home to 4 subnets (Note 01 §6), with lots of room to spare.
