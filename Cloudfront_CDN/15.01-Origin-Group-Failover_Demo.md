...
1. **Region selector** (top-right of the console) → switch to **Asia Pacific (Mumbai) `ap-south-1`** — the same Region your S3 bucket from the [CloudFront Hands-On Lab 1 (S3 static site + CDN)](02-CloudFront-HandsOn-Lab1.md) note is already in.
   > 🧠 CloudFront origins can technically live in **any** Region — an origin's physical location doesn't have to match another origin's, since CloudFront's edge locations reach out to whichever Region an origin is actually in. Using the same Region here is purely for this demo's convenience (one Region to keep track of, one place to look for both resources) — not a hard CloudFront requirement.
2. **EC2 console** → **Instances** → **Launch instances**.
3. **Name**: `CloudFront-Origin-StatusPage`.
...
6. **Key pair**: **Proceed without a key pair** — nothing in this demo requires SSH access; the page is fully built by the user data script at boot.
...
9. **Advanced details** → scroll to **User data** → paste:
   ```bash
   #!/bin/bash
   dnf install -y httpd
...
<img width="2562" height="1290" alt="image" src="https://github.com/user-attachments/assets/823ba78f-7a74-4b49-8915-349b8cdb1380" />
...
This file already exists at [`demo-site/status.html`](demo-site/status.html), right next to the rest of the [CloudFront Hands-On Lab 1 (S3 static site + CDN)](02-CloudFront-HandsOn-Lab1.md) note's site files — no need to type it by hand, just upload it as-is. Full content shown below for reference.
...
2. **Upload** → **Add files** → select `status.html` from `demo-site/` (must land at the bucket **root**, named **exactly** `status.html`):
   ```html
   <!DOCTYPE html>
   <html lang="en">
   <head>
...
<img width="2414" height="837" alt="image" src="https://github.com/user-attachments/assets/1ba275ef-25ed-4119-9caf-55204319bd17" />

<img width="2562" height="1290" alt="image" src="https://github.com/user-attachments/assets/782452a8-568d-4331-8749-021749282abc" />
...
<img width="2562" height="1283" alt="image" src="https://github.com/user-attachments/assets/1954233b-1320-45ff-8aec-1d73ec45eb3d" />
...
2. **Origins** → **Choose origins to add to group** dropdown → select `EC2StatusOrigin` → **Add**. Repeat, this time selecting the existing S3 origin from the [CloudFront Hands-On Lab 1 (S3 static site + CDN)](02-CloudFront-HandsOn-Lab1.md) note (its name will match your bucket, in `<bucket>.s3-website-<region>.amazonaws.com` form) → **Add**.
3. There's no dedicated "Primary"/"Secondary" field — **role is determined purely by list position**. Both origins now appear in a numbered list; whichever is listed as **1** is treated as the primary (the console labels it **"(primary)"** right next to the name), and **2** is the secondary. Use the **▲/▼** arrows next to each entry to reorder them if needed — confirm `EC2StatusOrigin` is at position **1** and the S3 origin is at position **2**.
4. **Name**: `StatusPageOriginGroup`.
5. **Failover criteria**: check `500 Internal server error`, `502 Bad gateway`, `503 Service unavailable`, `504 Gateway timeout`.
6. **Origin selection criteria**: leave at **Default** (the alternative, **Media quality score**, is for CMAF media-origin selection — not relevant here).
7. **Create origin group**.

<img width="2562" height="1290" alt="image" src="https://github.com/user-attachments/assets/d77120d3-d1ea-40d1-9464-1aab98a6d82b" />
...
<img width="2562" height="1290" alt="image" src="https://github.com/user-attachments/assets/7e7f7db9-c610-4eb9-9b57-789fbfb23cbe" />
...
<img width="2562" height="1367" alt="image" src="https://github.com/user-attachments/assets/1ed214c2-c41e-4a89-bdba-215ca2e62688" />
...
<img width="2562" height="1367" alt="image" src="https://github.com/user-attachments/assets/b92306fe-b4fe-4b41-8da0-cc6968b3e662" />
...
<img width="2562" height="1367" alt="image" src="https://github.com/user-attachments/assets/f286e30d-0ee6-409b-865c-19d238d8e202" />
