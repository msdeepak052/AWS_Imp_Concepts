...
> Goal: automate the predictable "more capacity during business hours, less overnight" pattern using **scheduled actions** on `demo-asg` — no metrics, no manual clicking, just date/time rules. Continues from manual scaling; next up is dynamic scaling, which reacts to *real* load instead of the clock.
...
## 4. Hands-on: two scheduled actions on `demo-asg`

We add two recurring scheduled actions to `demo-asg` (currently min 2 / desired 2 / max 6, no scaling policy):
...
1. **EC2 console** → **Auto Scaling Groups** → `demo-asg` → **Automatic scaling** tab → **Scheduled actions** section → **Create scheduled action**.
2. **Name**: `demo-scale-up-business-hours`.
...
2. **Name**: `demo-scale-down-evening`.
...
- At 8:00 AM IST Mon–Fri: `demo-scale-up-business-hours` sets min=4/desired=4 → the ASG launches 2 more instances (2 → 4), which register with `demo-tg` as they pass health checks.
- Through the day: capacity sits at 4 (nothing else is changing it, since no dynamic policy is attached in this note).
- At 8:00 PM IST Mon–Fri: `demo-scale-down-evening` sets min=2/desired=2 → the ASG deregisters and terminates 2 instances, chosen per the group's termination policy (a whole topic on its own, covered in dedicated notes later in this series), back down to 2.
...
```mermaid
flowchart LR
    subgraph Timeline["demo-asg desired capacity over one weekday (IST)"]
        direction LR
...
- **Auto Scaling Groups** → `demo-asg` → **Automatic scaling** tab → **Scheduled actions**: both actions listed with their next scheduled run time shown.
- Around 8:00 AM/8:00 PM IST, check the **Activity** tab for "Launching"/"Terminating" activities matching the schedule, and confirm `demo-tg`'s target count moves 2 → 4 → 2 accordingly.

---

## 5. Exam tip: scheduled vs dynamic scaling

🎯 **Exam tip:** **Scheduled scaling never looks at real load — it is purely time-based.** If the exam scenario says traffic is "predictable" or gives specific times/dates, the answer is scheduled scaling. If it says traffic is "variable," "unpredictable," or references a **CloudWatch metric/alarm** driving the change, that's **dynamic scaling** (covered next) instead. A well-designed production ASG often uses **both together** — e.g. scheduled actions raise the *minimum* baseline before an expected traffic ramp, while a target-tracking dynamic policy still handles moment-to-moment fluctuations within whatever min/max the schedule currently allows.
...
- Built two actions on `demo-asg`: `demo-scale-up-business-hours` (`0 8 * * 1-5`, IST) → min/desired 4; `demo-scale-down-evening` (`0 20 * * 1-5`, IST) → min/desired 2 — producing a 2 → 4 → 2 daily capacity curve on weekdays.
