...
> 🧠 **Mental model:** this is the same "modify traffic in flight, transparently to the origin" idea as the [CloudFront Origin Settings](03-CloudFront-Origin-Settings.md) note's custom request headers, just applied to the **response** side instead of the request side.
...
- **Viewer** — outside CloudFront entirely, wherever the user actually is (India, in the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's running example).
...
The important consequence: **the Response Headers Policy step runs on every single response, Hit or Miss alike, entirely at the edge** — it isn't part of what gets cached, it's applied fresh each time a response actually leaves that edge location. That has a very practical effect: editing a Response Headers Policy's header value takes effect on the **very next request**, no invalidation (the [Cache Invalidation](18-CloudFront-Cache-Invalidation.md) note) required, and no round trip to the origin either — unlike a change to the cached body itself, which only updates on a Miss or an explicit invalidation. The [Response Headers Policy hands-on demo](10.01-Response-Header-Policy_Demo.md)'s Section 7 verifies this directly: a cached object still reports `X-Cache: Hit` after a policy edit, while its headers have already changed.
...
## 4. Real-world walkthrough: applying this to the Cache Key and Origin Requests streaming example

Picking the same India-based Netflix-style session from the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's Section 3 back up, a Response Headers Policy would typically show up at two of its four stages:
...
| 3. Search "Inception" in Hindi | The `/api/search` responses, if the web app calls it from a separate subdomain (e.g. `www.netflix.com` calling `api.netflix.com`) | `Access-Control-Allow-Origin` scoped specifically to `www.netflix.com` — **not** `*`, since this is an authenticated, personalized response, and the CORS spec itself disallows combining wildcard origins with credentials (the same constraint the [Response Headers Policy hands-on demo](10.01-Response-Header-Policy_Demo.md)'s Section 6 hits when configuring `Access-Control-Allow-Credentials`) |
...
> ⚠️ A CORS header set at the S3 origin only actually reaches the browser if CloudFront **forwards** it unmodified — and only takes effect for requests CloudFront doesn't already have cached with a *different* (or missing) CORS header from an earlier response. This is a real, sometimes-confusing interaction between the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's caching rules and this note's CORS handling, and a common source of "CORS works sometimes but not always" bug reports in production CloudFront setups. It's the origin-header side of the caveat in Section 3 above — the CloudFront-policy side doesn't have this problem, since it re-applies on every response regardless of cache state.
...
For a full hands-on walkthrough — including a custom policy, CORS, and directly testing Section 3's Hit-but-headers-changed behavior — see the [Response Headers Policy hands-on demo](10.01-Response-Header-Policy_Demo.md).
...
- It sits **after** the [Cache Key and Origin Requests](09-Default-Cache-Behavior-Cache-Key-and-Origin-Requests.md) note's two-leg Cache Key/Origin Request Policy model, applying fresh on every response regardless of whether that response was a cache Hit or Miss — a genuinely different timeline from the cached body itself (Section 3).
...
- Next: the [CloudFront Function Associations](11-CloudFront-Function-Associations.md) note, the most powerful cache-behavior customization: running actual code at specific points in the request/response lifecycle.
