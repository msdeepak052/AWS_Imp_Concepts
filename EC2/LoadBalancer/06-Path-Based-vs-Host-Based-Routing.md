...
> Goal: understand **listener rules** — the mechanism that lets one ALB (`myapp-alb`, built in Note 05) split traffic across *multiple* backend services instead of forwarding everything to one target group. This note is concepts-only; Notes 07 and 08 build path-based and host-based routing for real on top of `myapp-alb`.

---

## 1. Recap: what `myapp-alb` looks like today

At the end of Note 05, `myapp-alb` has exactly one listener (`HTTP:80`) with exactly one rule: the **default rule**, which forwards every request to `myapp-tg` (the target group behind `myapp-asg`). There's no branching — every single request, regardless of URL or hostname, goes to the same place.

That's fine for a single service. But real applications often need one ALB to front **several** backend services — an API, an admin panel, a marketing site — without paying for (and managing DNS for) a separate load balancer per service. **Listener rules** are how you teach `myapp-alb` to look at each incoming request and decide *which* target group it should actually go to.

---

## 2. What is a listener rule?

A listener (e.g. `myapp-alb`'s `HTTP:80` listener) can hold **multiple rules**. Each rule (per AWS docs) has:
...
Every listener always has exactly one **default rule**: no conditions, can't be given a priority, and can't be deleted — it's simply whatever the last-resort action should be. In `myapp-alb`'s case, the default rule forwards to `myapp-tg`.
...
- If you add a broad rule at a **lower priority number** than a more specific one, the broad rule will "shadow" (steal traffic from) the specific rule — a classic misconfiguration bug (see the troubleshooting table in Note 07).
...
Example: everything under `myapp-alb`'s domain should mostly hit the normal web app, except calls under `/api/*`, which should go to a separate API service:

| Path | Routed to |
|---|---|
| `/`, `/home`, `/products` | `myapp-tg` (default rule) |
| `/api/anything` | `myapp-tg-api` |

This is a `path-pattern` condition. Path patterns are **case-sensitive**, support wildcards `*` (zero or more characters) and `?` (exactly one character), and match only against the path — never the query string. Note 07 builds this exact scenario against `myapp-tg-api`.
...
|---|---|
| `www.myapp.internal` / anything else | `myapp-tg` (default rule) |
| `admin.myapp.internal` | `myapp-tg-admin` |

Host header conditions are **not case-sensitive**, support the same `*`/`?` wildcards, and must contain at least one `.` (e.g. `*.myapp.internal` is valid, `myapp` alone is not). Note 08 builds this exact scenario against `myapp-tg-admin`.
...
> IF host-header = `admin.myapp.internal` **AND** path-pattern = `/reports/*` → forward to a reports-specific target group
...
## 8. Diagram: rule evaluation flow on `myapp-alb`

```mermaid
flowchart LR
    REQ(["Incoming request<br/>to myapp-alb HTTP:80"]) --> R1{"Rule priority 10:<br/>host-header =<br/>admin.myapp.internal ?"}
    R1 -->|"match"| TGADMIN["forward to<br/>myapp-tg-admin"]
    R1 -->|"no match"| R2{"Rule priority 20:<br/>path-pattern =<br/>/api/* ?"}
    R2 -->|"match"| TGAPI["forward to<br/>myapp-tg-api"]
    R2 -->|"no match"| DEFAULT["Default rule<br/>(always last, no priority)"]
    DEFAULT --> TG["forward to<br/>myapp-tg"]
```

Note the rule numbers here are illustrative of *relative* order (host rule checked before path rule) — Notes 07/08 assign the actual priority values used on `myapp-alb`.
...
- Next: Note 07 builds path-based routing for real — a new `myapp-tg-api` target group and demo instance `myapp-api-1`, with a `/api/*` rule added to `myapp-alb`.
